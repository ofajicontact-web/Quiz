...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, requestFullScreen, onAvatarChangeStart, showFullscreenPrompt, onShowProfile, currentPlayer, onStartGame }) => {
    ...
    const handleClosePinModal = () => {
         db.collection('players').where("username", "==", username).where("pin", "==", generatedPin).get()
            .then((querySnapshot: any) => {
                if (!querySnapshot.empty) {
                    const doc = querySnapshot.docs[0];
                    onLogin({ id: doc.id, ...doc.data() });
                    setStep('intro'); // Go back to intro screen, which will now show "Play" button
                }
            });
