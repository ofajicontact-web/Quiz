...
const QuestionCard: React.FC<QuestionCardProps> = ({ question, onAnswer, answerData, timerStartTime, onTimeUpdate, disabledOptions }) => {
    ...
    }, [question.id, isAnswered, timerStartTime]);
    ...
    return (
        <div className={`relative bg-slate-800/50 backdrop-blur-sm rounded-2xl shadow-2xl p-4 sm:p-6 border-2 border-slate-700 h-full flex flex-col`}>
            ...
            <div className="h-[140px] sm:h-[160px] flex items-center justify-center mb-4 overflow-hidden">
                <h2 ref={questionRef} className="text-base sm:text-lg font-semibold leading-tight text-center" dangerouslySetInnerHTML={{ __html: question.question[language] }} />
            </div>

            <div className="grid grid-cols-2 gap-2 sm:gap-3 flex-1">
                {optionsToRender.map((option, index) => (
                    <button key={index} onClick={() => handleAnswerClick(option)} disabled={isAnswered || disabledOptions.includes(option)}
                        className={`py-2 px-3 sm:py-3 sm:px-4 text-sm sm:text-base rounded-lg border-2 font-semibold transition-all duration-300 transform disabled:transform-none disabled:cursor-not-allowed h-full flex items-center justify-center text-center ${getButtonClass(option)}`}
                    >
